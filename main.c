...
static const char base_piece_x[7][4] = {
	{ -1, 0, 1, 2 },	// line
	{ -1, 0, 1, 1 },	// backwards L
	{ -1, -1, 0, 1 },	// L
	{ 0, 1, 1, 0 },		// square
...
static const char base_piece_y[7][4] = {
	{ 0, 0, 0, 0 },		// line
	{ 0, 0, 0, -1 },	// backwards L
	{ -1, 0, 0, 0 },	// L
	{ 0, 0, -1, -1 },	// square
...
static void blit(const Sint16 x, const Sint16 y, SDL_Surface * s)
{
	SDL_Rect rect;
	rect.x = x;
	rect.y = y;
...
static void blit_number(const Sint16 x, const Sint16 y, int number)
{
	SDL_Rect src;
	SDL_Rect dst;
	src.x = src.y = 0;
...
static void draw_piece(const Sint16 x, const Sint16 y,
	const char piece_x[4], const char piece_y[4], SDL_Surface * img)
{
	for (int i = 0; i < 4; i++) {
		Sint16 px = x + 24 * piece_x[i];
		Sint16 py = y + 24 * piece_y[i];
...
static int test_overlap(char board[20][10], const Sint16 x, const Sint16 y,
	const char piece_x[4], const char piece_y[4])
{
	for (int i = 0; i < 4; i++) {
...
//					needs_flip = 1;
				...
				if (field_dirty) {
					...
					// draw saved board
					for (Sint16 j = 0; j < 20; j++)
						for (Sint16 i = 0; i < 10; i++)
